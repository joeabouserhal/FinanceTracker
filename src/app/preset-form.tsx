...
export default function PresetForm() {
  ...
  const toggleBtn = (active: boolean, t: string) => ({
    flex: 1,
    borderWidth: 2,
    borderColor: active ? (t === "income" ? theme.income : theme.expense) : theme.muted,
    backgroundColor: active ? (t === "income" ? theme.income : theme.expense) : "transparent",
    paddingVertical: 10,
  ...
  const handleSave = async () => {
    if (!name.trim()) { showAlert("Name required", "Give the preset a name."); return; }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      {/* Sticky Header */}
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 16, paddingTop: 48, paddingBottom: 16, backgroundColor: theme.background }}>
        ...
        <TouchableOpacity onPress={handleSave} disabled={saving}>
          {saving ? <ActivityIndicator color={theme.accent} /> : <T variant="body" style={{ color: theme.accent, fontSize: 14, textTransform: "uppercase" }}>Save</T>}
        </TouchableOpacity>
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 16 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ flexDirection: "row", marginBottom: 20, paddingTop: 8 }}>
          {(["expense", "income"] as const).map((t) => (
            <TouchableOpacity key={t} onPress={() => setType(t)} style={toggleBtn(t === type, t)}>
              <T variant="body" style={{ color: t === type ? theme.background : theme.muted, fontSize: 14, textTransform: "uppercase", fontWeight: "700" }}>{t}</T>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <TextInput style={inputStyle} placeholder="e.g. Coffee, Netflix" placeholderTextColor={theme.muted} value={name} onChangeText={setName} />

        {/* Default Amount */}
        <T variant="label" style={{ marginTop: 20, marginBottom: 6 }}>Default Amount (optional)</T>
        <TextInput style={inputStyle} placeholder="0.00" placeholderTextColor={theme.muted} keyboardType="decimal-pad" value={defaultAmount} onChangeText={setDefaultAmount} />
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
          {currencies?.map((c) => (
            <TouchableOpacity key={c.id} onPress={() => setCurrencyId(c.id)} style={chip(currencyId === c.id)}>
              <T variant="label" style={{ color: currencyId === c.id ? theme.background : theme.muted, fontSize: 13 }}>{c.code}</T>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {filteredCategories.map((c) => (
            <TouchableOpacity key={c.id} onPress={() => setCategoryId(c.id)} style={[chip(categoryId === c.id), categoryId === c.id && c.color ? { borderColor: c.color, backgroundColor: c.color } : {}]}>
              <T variant="label" style={{ color: categoryId === c.id ? theme.background : theme.muted, fontSize: 13 }}>{c.name}</T>
            </TouchableOpacity>
          ))}
        </ScrollView>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
          <TouchableOpacity onPress={() => setAccountId("")} style={chip(!accountId)}>
            <T variant="label" style={{ color: !accountId ? theme.background : theme.muted, fontSize: 13 }}>None</T>
          </TouchableOpacity>
          {accounts?.filter((a) => !a.archived).map((a) => (
            <TouchableOpacity key={a.id} onPress={() => setAccountId(a.id)} style={chip(accountId === a.id)}>
              <T variant="label" style={{ color: accountId === a.id ? theme.background : theme.muted, fontSize: 13 }}>{a.name}</T>
            </TouchableOpacity>
          ))}
        </View>
        ...
        {isEdit && (
          <>
            ...
            <TouchableOpacity
              style={{ borderWidth: 2, borderColor: theme.expense, paddingVertical: 12, alignItems: "center" }}
              onPress={() => setArchiveVisible(true)}
            >
              <T variant="body" style={{ color: theme.expense, textTransform: "uppercase", fontSize: 14 }}>Archive Preset</T>
            </TouchableOpacity>
          </>
        )}
