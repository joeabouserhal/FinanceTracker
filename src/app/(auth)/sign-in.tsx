  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: theme.background }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <ScrollView contentContainerStyle={{ flex: 1, justifyContent: "center", padding: 24 }} keyboardShouldPersistTaps="handled">
        ...
        <T variant="body" style={{ color: theme.muted, fontSize: 16, marginBottom: 40 }}>Sign in to your account</T>
        ...
        <Controller control={control} name="password" render={({ field: { onChange, onBlur, value } }) => (
          <TextInput style={[inputStyle, errors.password ? { borderColor: theme.expense } : {}]} placeholder="Your password" placeholderTextColor={theme.muted} secureTextEntry autoComplete="password" onBlur={onBlur} onChangeText={onChange} value={value} />
        )} />
        ...
        <TouchableOpacity style={{ backgroundColor: theme.accent, paddingVertical: 16, alignItems: "center", marginTop: 28, borderWidth: 2, borderColor: theme.accent }} onPress={handleSubmit(onSubmit)} disabled={loading}>
          {loading ? <ActivityIndicator color={theme.background} /> : <T variant="body" style={{ color: theme.background, fontSize: 16, fontWeight: "700", textTransform: "uppercase" }}>Sign In</T>}
        </TouchableOpacity>

        <Link href="/(auth)/sign-up" style={{ color: theme.accent, fontSize: 14, textAlign: "center", marginTop: 24, textDecorationLine: "underline", fontFamily: "IBMPlexSans" }}>
